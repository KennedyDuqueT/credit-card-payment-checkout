...
export class PaymentsController {
  ...
  @Post('process')
  async processPayment(@Body() createTransactionDto: CreateTransactionDto): Promise<PaymentResponseDto> {
    return this.paymentsService.processPayment(createTransactionDto);
  }

  @Get('status/:transactionNumber')
  async getTransactionStatus(@Param('transactionNumber') transactionNumber: string): Promise<Transaction> {
